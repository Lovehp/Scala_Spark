...
object HbClient {
  def createHTable(connection: Connection,tablename: String,familyName:String): Unit=
  {
    //Hbase表模式管理器
    ...
    if (!admin.tableExists(tableName)) {
      ...
      //创建列簇
      tableDescriptor.addFamily(new HColumnDescriptor(familyName.getBytes()))
      //创建列簇2    author
  ...
  //删除某条记录
  def truncateTab(connection:Connection,tablename:String): Unit ={
    //本例将操作的表名
    val tableName = TableName.valueOf(tablename)
    //Hbase表模式管理器
    val admin = connection.getAdmin
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tablename = "yudi_hb_user360"
    conf.set("hbase.zookeeper.property.clientPort", "2181")
    //conf.set("zookeeper.znode.parent", "/hbase-unsecure")
    conf.set("hbase.zookeeper.quorum", "c3pretest1:2181,c3pretest2:2181,c3pretest3:2181")
    ...
    try{
      ...
      try {
        ...
        truncateTab(connection, tablename)
